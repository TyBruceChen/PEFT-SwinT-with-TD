...
from cara.cara_swt import cara_swt
#from src.cara.cara_swt import cara_swt

best_acc = 0.0

def train(args, model, dl, tdl, opt, sched, epochs, criterion= th.nn.CrossEntropyLoss()):
    model.train()
    model = model.cuda()
    acc = 0.
    global old_name
    ...
    for epoch in (pbar:=tqdm(range(epochs))):
        ...
        for batch in dl:
            if log and args.trainable == 'LM':
                r1_hist = wandb.Histogram(swt.CP_R1.cpu().detach().numpy())
                r2_hist = wandb.Histogram(swt.CP_R2.cpu().detach().numpy())
                logger.log({
                    "R1": r1_hist,
                    "R2": r2_hist
                })
                logger.log({
                    "r1_mean": th.mean(swt.CP_R1.cpu()),
                    "r2_mean": th.mean(swt.CP_R2.cpu())
                })
                logger.log({
                    "r1_std": th.std(swt.CP_R1.cpu()),
                    "r2_std": th.std(swt.CP_R2.cpu())
                })
        ...
        if epoch % 3 == 0 and epoch != 0:
            ...
            if acc > args.best_acc:
                ...
                if old_name is not None:
                    os.remove(old_name)
                old_name = f"./swt_{args.dataset}_{round(acc, 5)}_seed_{args.seed}.pt"
...
def _parse_args():
    ...
    parser.add_argument('--trainable', type=str, default='LM',
                        help='default:LM for low ranking matrix, others: fft')
...
def main(sd = None):
    ...
    if log:
        run_name = f"LR_{args.lr}-Rank_{args.dim}_{dataset_name}_{args.trainable}" #WM-811K-wafer-map"
    ...
    global swt
    import timm
    swt = timm.create_model(args.model, checkpoint_path= args.check_point_path_swt, num_classes=num_classes, pretrained = True)
    cara_config = {
        "model": swt,
        "rank": args.dim,
        "scale": scale,
    ...
    if args.trainable == 'LM':
        swt = cara_swt(cara_config) #Low ranking matrix for DT injection
